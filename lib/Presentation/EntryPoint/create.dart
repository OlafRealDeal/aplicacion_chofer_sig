...
class CreateEntryPoint extends StatefulWidget{
  const CreateEntryPoint({Key? key}) : super(key: key);
  @override
  State<StatefulWidget> createState() {
    return __CreateEntryPointState();
  }

}

class __CreateEntryPointState extends State<CreateEntryPoint>{
          ...
          child: Container(
            ...
            child: Column(
              children: <Widget>[
                ...
                SizedBox(height: 10,),
                buttonSeccondary(context, "Guardar", storeEntryPoint),
                SizedBox(height: 10,),
  ...
  Future<void> storeEntryPoint() async{
    if (_formKey.currentState!.validate()) {
      showLoadingIndicator(context,'Creating EntryPoint...');

      DataResponse dataResponse=await entrypointBusiness.store(lat,long,bus_route_id);
      ...
      setState(() {
        ...
        if (dataResponse.status){
          ...
          showAlertDialog(context, "Error to storage EntryPoint", dataResponse.message);
        }
      });
    }
  }
