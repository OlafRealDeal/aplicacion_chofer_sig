...
class EditEntryPointArguments {
  final EntryPoint entrypoint;

  EditEntryPointArguments(this.entrypoint);
}

class EditEntryPoint extends StatefulWidget{
  const EditEntryPoint({Key? key, }) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return __EditEntryPointState();
  }

}

class __EditEntryPointState extends State<EditEntryPoint>{
  ...
  Widget build(BuildContext context) {
    ...
    if (isInitState){
      isInitState=false;
      final args = ModalRoute.of(context)!.settings.arguments as EditEntryPointArguments;
      this.entrypoint=args.entrypoint;

      controllerlat.text=args.entrypoint.lat;
      controllerlat.addListener(() {
        entrypoint.lat = controllerlat.text;
      });
      controllerlong.text=args.entrypoint.long;
      controllerlong.addListener(() {
        entrypoint.long = controllerlong.text;
      });
  ...
  Widget cardEntryPoint(BuildContext context){
    return Card(
      ...
      child:Container(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 5,),
            idDropdown(context,entrypoint.bus_route_id,bus_route_idList,bus_route_idMap,bus_route_idState),
  ...
  Future<void> update() async{
    if (_formKey.currentState!.validate()) {
      showLoadingIndicator(context,'Updating EntryPoint...');

      DataResponse dataResponse=await entrypointBusiness.update(this.entrypoint);
      ...
      setState(() {
        ...
        if (dataResponse.status){
          ...
          showAlertDialog(context, "Error updating EntryPoint", dataResponse.message);
        }
      });
    }
  }
